...
def main(message):
    bot.send_message(message.chat.id, 'Артем краш')

@banan.message_handler(commands={"Да ?"})
def main(message):
    bot.send_message(message.chat.id, '*А вы не верили* \n а я уверен в себе',parse_mode='Markdown')

@banan.message_handler(commands={"Нет"})
def main(message):
    bot.send_message(message.chat.id, 'я хотел сделать выбор Да либо Нет ')
